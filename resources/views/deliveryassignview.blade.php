...
<body>
    <div class="container-fluid position-relative d-flex p-0">
        ...
        <div class="content">
            ...
            <!-- Navbar End -->
             <h1 align="center" style="color:red;">ASSIGN ORDER TO PARTNER </h1>
            <!-- Recent Sales Start -->
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h3 class="mb-0" >ASSIGN ORDER</h3>
                    </div>
                       ...
                       <table class="table text-start align-middle table-bordered table-hover mb-0">
                            <thead align="center">
                                <tr class="text-white">
                                    ...
                                    <th scope="col">CHOOSE PARTNER</th>
                                    <th scope="col">ASSIGN</th>
                                   
                                </tr>
                            </thead>
                            @foreach ($ALLODER->all() as $item)
                            <tbody align="center">
                                <tr>
                               <form action="{{url('/assignorderaction')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <td><input type="hidden" name="doid" value="{{$item->User_id}}"></td>
                                   <td>{{$item->Order_id}}
                                    <input type="hidden" name="oid" value="{{$item->Order_id}}">
                                   </td>
                                   <td>{{$item->Name}}
                                   ...
                                   <td>{{$item->Address}},{{$item->City}},{{$item->Pin}},{{$item->State}}
                                   <input type="hidden" name="address" value="{{$item->Address}},{{$item->City}},{{$item->Pin}},{{$item->State}}">
                                   </td>
                                   <td>{{$item->P_name}}
                                   <input type="hidden" name="p_name" value="{{$item->P_name}}">
                                   </td>
                                   <td>{{$item->P_qty}}
                                   <input type="hidden" name="p_qty" value="{{$item->P_qty}}">
                                   </td>
                                   <td>{{$item->P_size}}
                                   <input type="hidden" name="p_size" value="{{$item->P_size}}">
                                   </td>
                                   <td>{{$item->P_price}}
                                   ...
                                   <td><button type="submit" class="btn btn-sm btn-primary">Assign</button></td>
...
@if (isset($Finfo))
    <?php
        ...
        $deliveredOrder = $Finfo->where('Status', 'Deliver')->count();
    ?>
    <div class="container-fluid pt-4 px-4">
        <div class="row g-4">
                ...
                <div class="bg-secondary rounded-top p-4">
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-6 text-center text-sm-end">
                            ...
                            Designed By <a href="#">Ritam Samanta</a>
                        </div>
                    </div>
                </div>
            </div>
